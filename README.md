...
- The `development` branch is highly dynamic, has its own automated CI workflow, _no live deployment_, and only accepts commits through pull requests. In order for a front-end dev to work in tandem with a back-end dev, we can do one of the following:
  1.  The front-end developer can change the client-side `BASE_API_URL` in their `.env` to match the preview deployment URL generated by the server's hosting platform. Render, Railway, and Fly.io all offer preview deployments. This is a major win over traditional hosting platforms like AWS, which would otherwise necessitate complex orchestration to make preview deployments possible.
